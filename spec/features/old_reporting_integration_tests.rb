class ReportingAPIIntegrationTest < Test::Unit::TestCase
  ...
  should "have successfully found my payments" do
    ...
    #make a 1st purchase
    result = Bambora.PaymentsAPI.make_payment(purchase)
    ...
    results = Bambora.ReportingAPI.search_transactions(last3Hours, next3Hours, 1, 3)
    assert(results != nil)
    assert(results.length == 3)


    #find transaction 1 from order number
    results = Bambora.ReportingAPI.search_transactions(
      last3Hours,
      next3Hours,
      1,
      10,
    ...
    results = Bambora.ReportingAPI.search_transactions(last3Hours, next3Hours, 1, 10,
      Array[
