class ProfilesAPIIntegrationTest < Test::Unit::TestCase
  ...
  setup do
    ...
    @profile = Bambora.ProfilesAPI.create_profile(
    {
      :billing => {
        :name => "Hilary Test",
...
# Profile CREATE with CARD
  should "have successfully created a profile with credit card" do
    ...
    begin
      profile = Bambora.ProfilesAPI.getCreateProfileWithCardTemplate()
      ...
      result = Bambora.ProfilesAPI.create_profile(profile)
  ...
  should "have successfully created a profile with token" do
    ...
    begin
      token = Bambora.PaymentsAPI.get_legato_token(
        {
          :number => "4030000010001234",
          :expiry_month => "07",
          :expiry_year => "22",
      ...
      profile = Bambora.ProfilesAPI.getCreateProfileWithTokenTemplate()
...
# Profile DELETE
  should "have successfully deleted a profile" do
    ...
    result = Bambora.ProfilesAPI.create_profile(
    {
      :card => {
        :name => "Jill Test",
        :number => "4030000010001234",
    ...
    result = Bambora.ProfilesAPI.delete_profile(profile_id)
...
# Profile GET
  should "have successfully retrieved a profile" do
    ...
    result = Bambora.ProfilesAPI.get_profile(profile_id)
    assert(result != nil)
    assert(result['billing']['name'] == "Jill Test")

    Bambora.ProfilesAPI.delete_profile(profile_id) #delete it to clean up
  end
  ...
  should "have successfully updated a profile" do
    ...
    # now get that profile
    profile1 = Bambora.ProfilesAPI.get_profile(profile_id)
    assert(profile1 != nil)
    ...
    result = Bambora.ProfilesAPI.update_profile(profile1)

    #get profile again and check the updates
    profile2 = Bambora.ProfilesAPI.get_profile(profile_id)
    puts "\n\nRetrieved Profile 2:\n\n #{profile2}"
    assert(profile2 != nil)
  ...
  should 'Successfully Add Card' do
    insert_card = Bambora.ProfilesAPI.add_profile_card(@profile,@card1)
    #Retrieve Profile  again to check if card added
    profile2 = Bambora.ProfilesAPI.get_profile(@profile['customer_code'])
    assert(profile2['card'])
    assert(insert_card['message']== 'Operation Successful')
  end

  should 'Successfully get Card' do
    add_card = Bambora.ProfilesAPI.add_profile_card(@profile,@card1)
    card = Bambora.ProfilesAPI.get_profile_card(@profile)
    assert(card)
  ...
  should 'successfully update card' do
    ...
    insert_card = Bambora.ProfilesAPI.add_profile_card(@profile,@card2)
    # Update profile with card 2
    update_card = Bambora.ProfilesAPI.update_profile_card(@profile,1,@card1)
    profile2 = Bambora.ProfilesAPI.get_profile(@profile['customer_code'])
  ...
  should 'successfully delete card' do
    ...
    delete_card = Bambora.ProfilesAPI.delete_profile_card(@profile,1)
  ...
  should "have successfully made payments with a profile" do
    ...
    begin
      # payment
      profile_payment = Bambora.PaymentsAPI.getProfilePaymentRequestTemplate()
      profile_payment[:payment_profile][:customer_code] = profile_id
      profile_payment[:amount] = 77.50
      result = Bambora.PaymentsAPI.make_payment(profile_payment)
    rescue BamboraException => ex
      puts "Error: #{ex.user_facing_message()}"
    ...
    begin
      ...
      #complete pre-auth
      result = Bambora.PaymentsAPI.complete_preauth(result['id'], 40.50)
